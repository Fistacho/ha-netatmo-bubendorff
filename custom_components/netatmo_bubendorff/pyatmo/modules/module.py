...
class ShutterMixin(EntityBase):
    ...
    Bubendorff shutters accept 4 discrete target_position values:
       100 = open, 0 = closed, -1 = stop, -2 = preferred (jalousie/slats).
    """

    # Special target_position values accepted by Netatmo API for Bubendorff.
    POSITION_OPEN = 100
    POSITION_CLOSED = 0
    POSITION_STOP = -1
    POSITION_PREFERRED = -2  # jalousie / slats mode
    ...
    async def async_set_target_position(self, target_position: int) -> bool:
        """Set shutter to target position (must be 0, 100, -1, or -2)."""
    ...
    async def async_open(self) -> bool:
        ...
        return await self.async_set_target_position(self.POSITION_OPEN)

    async def async_close(self) -> bool:
        """Close shutter fully."""

        return await self.async_set_target_position(self.POSITION_CLOSED)

    async def async_stop(self) -> bool:
        """Stop shutter movement."""

        return await self.async_set_target_position(self.POSITION_STOP)

    async def async_open_tilt(self) -> bool:
        """Move shutter to preferred (jalousie) position — slats tilted."""

        return await self.async_set_target_position(self.POSITION_PREFERRED)

    async def async_close_tilt(self) -> bool:
        """Exit jalousie — return slats flat (shutter fully closed)."""
